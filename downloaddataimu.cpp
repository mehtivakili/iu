...
<body>
  ...
  <script>
    let ws;
    let collectedData = [];
    const collectionDuration = 50000; // 50 seconds
    const collectionInterval = 50; // 50mili second
    const defaultConfig = {
    ...
    function initializeWebSocket() {
      ...
      ws.onmessage = function(event) {
        ...
        collectedData.push([time, accelX, accelY, accelZ, gyroX, gyroY, gyroZ]);

        if (collectedData.length > collectionDuration / collectionInterval) {
            collectedData.shift();
        }
        } catch (e) {
