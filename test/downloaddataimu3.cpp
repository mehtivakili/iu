...
<body>
  <div class="config-section">
    ...
    <input type="number" id="collectionInterval" name="collectionInterval" min="50" step="10" value="100">
    <br>
  ...
  let collectionInterval = 50; // Default 100 milliseconds
    const defaultConfig = {
...
ws.onmessage = function(event) {
  ...
  try {
    ...
    const formattedTime = time.toFixed(2); // Format time to two decimal places

    const newRow = document.createElement('tr');
    newRow.innerHTML = `<td>${formattedTime}</td><td>${accelX}</td><td>${accelY}</td><td>${accelZ}</td><td>${gyroX}</td><td>${gyroY}</td><td>${gyroZ}</td>`;

    const dataBody = document.getElementById('dataBody');
    dataBody.insertBefore(newRow, dataBody.firstChild);

    // Collect data for download
    collectedData.push([formattedTime, accelX, accelY, accelZ, gyroX, gyroY, gyroZ]);
