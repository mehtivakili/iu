...
WiFiManager::WiFiManager() : server(443) {}  // Initialize AsyncWebServer on port 443
...
void WiFiManager::begin() {
    ...
    // Read certificate and key from SPIFFS
    File certFile = SPIFFS.open("/server.crt", "r");
    if (!certFile) {
    ...
    File keyFile = SPIFFS.open("/server.key", "r");
    if (!keyFile) {
    ...
    if (!connectToSavedWiFi()) {
        ...
        if (MDNS.begin("esp32")) {
            ...
            Serial.println("You can access the device at https://esp32.local");
        } else {
            Serial.println("Error setting up mDNS responder!");
        }

        server.on("/", HTTP_GET, [](AsyncWebServerRequest *request){
            request->send(200, "text/html", welcomePage);
        });

        server.on("/forgetWiFi", HTTP_POST, [this](AsyncWebServerRequest *request) {
...
void WiFiManager::handleClient() {
    // No need to manually handle clients with AsyncWebServer
}
    ...
    if (request->method() == HTTP_POST) {
        ...
        for (int i = 0; i < 10; i++) {
            if (WiFi.status() == WL_CONNECTED) {
                ...
                request->send(200, "text/html", "Wi-Fi credentials saved. Rebooting...");
                delay(2000);
                ESP.restart();
                return;
            }
...
void WiFiManager::handleForgetWiFi(AsyncWebServerRequest *request) {
