    ...
    ws.onmessage = function(event) {
      ...
      try {
        ...
        if (accelChart.data.labels.length > 200) {
          accelChart.data.labels = accelChart.data.labels.filter((_,index)=>index != 0);
          gyroChart.data.labels = gyroChart.data.labels.filter((_,index)=>index != 0);
          accelChart.data.datasets.forEach(dataset => dataset.data.filter((_,index)=>index != 0));
          gyroChart.data.datasets.forEach(dataset => dataset.data.filter((_,index)=>index != 0));
        }
